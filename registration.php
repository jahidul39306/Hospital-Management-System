<?php
    ...
    $data = json_decode(file_get_contents(DOCTOR_INFO));
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        ...
        if(search_username($data, $_POST["username"]))
        {
        ...
        if(!$hasErr){
            ...
            $data[] = array("fname" => $fname, "lname" => $lname, "gender" => $gender,
                "dob" => $dob, "religion" => $religion, "specialist" => $specialist,
                "presAddress" => $presAddress, "permAddress" => $permAddress,
                "phone" => $phone, "email" => $email, "website" => $website,
                "username" => $username, "password" => $password);
            $data = json_encode($data, JSON_PRETTY_PRINT);
            $result = file_put_contents(DOCTOR_INFO, $data);
            if($result)
            {
